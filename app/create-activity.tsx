...
const ACTIVITY_OPTIONS = ACTIVITIES.map((a) => ({ label: a.name, value: a.id }));
const LEVEL_OPTIONS = EXPERTISE_LEVELS.map((l) => ({ label: l.label, value: l.value }));
...
export default function CreateActivityScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedActivity) {
      showToast.error('Missing Field', 'Please select an activity (e.g. Football, Cricket).');
      return;
    }

    if (!selectedLevel) {
      showToast.error('Missing Field', 'Please select a level (Beginner, Intermediate or Pro).');
      return;
    }
    ...
    try {
      ...
      const errorMessage = error?.message || error?.code || 'Failed to create activity. Please check your connection and try again.';
      showToast.error(
        'Creation Failed',
        errorMessage
      );
        ...
        <KeyboardAvoidingView
            ...
            <TouchableOpacity
              ...
              {location ? (
                <View style={styles.locationSelected}>
                  <MaterialCommunityIcons name="map-marker" size={20} color="#0a7ea4" />
                ...
                <View style={styles.locationPlaceholder}>
                  ...
                  <Text
                    style={[
                      ...
                      isDark ? styles.mapButtonTextDark : styles.mapButtonTextLight,
                    ]}>
          ...
          <LocationMapPicker
            ...
            onLocationSelect={(loc) => {
              ...
              const isCoordinatesOnly = /^-?\d+\.?\d*,\s*-?\d+\.?\d*$/.test(loc.address || '');
